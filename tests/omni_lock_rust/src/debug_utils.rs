...
pub fn debug<
    'a,
    DL: CellDataProvider + ExtensionProvider + HeaderProvider + Send + Sync + Clone,
>(
    addr: &str,
    ...
    //             .generate_syscalls(script_version, script_group, Default::default())
    //             .into_iter()
    //             .fold(builder, |builder, syscall| builder.syscall(syscall));
    //         let mut machine = AsmMachine::new(builder.build());
...
//     DL: CellDataProvider + ExtensionProvider + HeaderProvider + Send + Sync + Clone,
// >(
//     verifier: &'a TransactionScriptsVerifier<DL>,
//     group_type: ScriptGroupType,
//     hash: &Byte32,
// ) -> Option<&'a ScriptGroup> {
//     for (h, g) in verifier.groups() {
//         if group_type == g.group_type && h == hash {
//             return Some(g);
//         }
//     }
//     None
