...
use misc::{
    assert_script_error, build_always_success_script, build_resolved_tx, debug_printer, gen_tx, sign_tx, verify_tx,
    DummyDataLoader, EthereumConfig, TestConfig, ALWAYS_SUCCESS, CKB_INVALID_DATA, ERROR_BURN, ERROR_EXCEED_SUPPLY,
...
fn sudt_type_script(loader: &DummyDataLoader, tx: &TransactionView) -> Script {
    ...
    let omni_lock_hash =
        loader.cells.get(&omni_lock_out_point).map(|(output, _)| output.lock().calc_script_hash()).unwrap();
...
fn add_sudt_to_inputs(
    ...
    loader.cells.insert(previous_out_point.clone(), (previous_output, previous_data));
    tx.as_advanced_builder().input(CellInput::new(previous_out_point, 0)).build()
}

fn add_sudt_to_outputs(sudt_type_script: Script, tx: TransactionView, amount: u128) -> TransactionView {
    ...
    tx.as_advanced_builder().output(output).output_data(data.pack()).build()
}

fn add_sudt_dep(loader: &mut DummyDataLoader, tx: TransactionView) -> TransactionView {
    let dep_out_point = gen_out_point();
    let sudt_cell =
        CellOutput::new_builder().capacity(Capacity::bytes(SIMPLE_UDT.len()).expect("script capacity").pack()).build();
    loader.cells.insert(dep_out_point.clone(), (sudt_cell, SIMPLE_UDT.clone()));
    tx.as_advanced_builder()
        .cell_dep(CellDep::new_builder().out_point(dep_out_point).dep_type(DepType::Code.into()).build())
        .build()
}
...
fn add_info_cell_to_outputs(type_script: Script, tx: TransactionView, data: Vec<u8>) -> TransactionView {
    let output = CellOutput::new_builder()
        .capacity(42.pack())
        .type_(Some(type_script).pack())
        .lock(build_always_success_script())
        .build();
    tx.as_advanced_builder().output(output).output_data(Bytes::from(data).pack()).build()
}

fn build_info_cell_data(current_supply: u128, max_supply: u128, sudt_type_script_hash: &[u8]) -> Vec<u8> {
    let version = 0u8;
...
fn test_success_issue_token() {
    run_sudt_supply_case(0, |data_loader, config| {
        ...
        let input_info_cell_data = build_info_cell_data(20, max_supply, sudt_type_script_hash.as_slice());
        let output_info_cell_data =
            build_info_cell_data(20 + issue_amount, max_supply, sudt_type_script_hash.as_slice());
...
fn test_burn_token() {
    ...
    run_sudt_supply_case(ERROR_BURN, |data_loader, config| {
        ...
        let input_info_cell_data = build_info_cell_data(400, max_supply, sudt_type_script_hash.as_slice());
        let output_info_cell_data = build_info_cell_data(200, max_supply, sudt_type_script_hash.as_slice());
        tx = add_info_cell_to_inputs(data_loader, info_cell_type_script.clone(), tx, input_info_cell_data);
        tx = add_info_cell_to_outputs(info_cell_type_script.clone(), tx, output_info_cell_data);
        tx
    });
...
fn test_too_many_info_cell_in_inputs() {
    ...
    run_sudt_supply_case(CKB_INVALID_DATA, |data_loader, config| {
        ...
        let output_info_cell_data =
            build_info_cell_data(20 + issue_amount, max_supply, sudt_type_script_hash.as_slice());
        tx = add_info_cell_to_inputs(data_loader, info_cell_type_script.clone(), tx, input_info_cell_data.clone());
        tx = add_info_cell_to_inputs(data_loader, info_cell_type_script.clone(), tx, input_info_cell_data);
        tx = add_info_cell_to_outputs(info_cell_type_script.clone(), tx, output_info_cell_data);
        tx
    });
...
fn test_too_many_info_cell_in_outputs() {
    ...
    run_sudt_supply_case(CKB_INVALID_DATA, |data_loader, config| {
        ...
        let output_info_cell_data =
            build_info_cell_data(20 + issue_amount, max_supply, sudt_type_script_hash.as_slice());
        tx = add_info_cell_to_inputs(data_loader, info_cell_type_script.clone(), tx, input_info_cell_data);
        tx = add_info_cell_to_outputs(info_cell_type_script.clone(), tx, output_info_cell_data.clone());
        tx = add_info_cell_to_outputs(info_cell_type_script.clone(), tx, output_info_cell_data);
        tx
    });
}
...
fn test_info_cell_size_not_enough() {
    ...
    run_sudt_supply_case(CKB_INVALID_DATA, |data_loader, config| {
        ...
        tx = add_sudt_dep(data_loader, tx);
        tx = add_sudt_to_outputs(sudt_type_script.clone(), tx, issue_amount);
        let mut input_info_cell_data = build_info_cell_data(20, max_supply, sudt_type_script_hash.as_slice());
        let mut output_info_cell_data =
            build_info_cell_data(20 + issue_amount, max_supply, sudt_type_script_hash.as_slice());
...
fn test_sudt_amount_overflow() {
    ...
    run_sudt_supply_case(ERROR_SUPPLY_AMOUNT, |data_loader, config| {
        ...
        let input_info_cell_data = build_info_cell_data(20, max_supply, sudt_type_script_hash.as_slice());
        let output_info_cell_data = build_info_cell_data(40, max_supply, sudt_type_script_hash.as_slice());
        tx = add_info_cell_to_inputs(data_loader, info_cell_type_script.clone(), tx, input_info_cell_data);
        tx = add_info_cell_to_outputs(info_cell_type_script.clone(), tx, output_info_cell_data);
        tx
    });
...
fn test_input_exceed_supply() {
    ...
    run_sudt_supply_case(ERROR_EXCEED_SUPPLY, |data_loader, config| {
        ...
        tx = add_sudt_dep(data_loader, tx);
        tx = add_sudt_to_outputs(sudt_type_script.clone(), tx, issue_amount);
        let input_info_cell_data = build_info_cell_data(6001, max_supply, sudt_type_script_hash.as_slice());
        let output_info_cell_data =
            build_info_cell_data(6001 + issue_amount, max_supply, sudt_type_script_hash.as_slice());
        tx = add_info_cell_to_inputs(data_loader, info_cell_type_script.clone(), tx, input_info_cell_data);
        tx = add_info_cell_to_outputs(info_cell_type_script.clone(), tx, output_info_cell_data);
        tx
    });
...
fn test_wrong_supply_delta() {
    ...
    run_sudt_supply_case(ERROR_SUPPLY_AMOUNT, |data_loader, config| {
        ...
        let input_info_cell_data = build_info_cell_data(20, max_supply, sudt_type_script_hash.as_slice());
        let output_info_cell_data =
            build_info_cell_data(20 + issue_amount + 2, max_supply, sudt_type_script_hash.as_slice());
        tx = add_info_cell_to_inputs(data_loader, info_cell_type_script.clone(), tx, input_info_cell_data);
        tx = add_info_cell_to_outputs(info_cell_type_script.clone(), tx, output_info_cell_data);
        tx
    });
